...
const BookManagement = () => {
  ...
  const { addBookPopup, readBookPopup, recordBookPopup, returnBookPopup } = useSelector((state) => state.popup)
  const {
    ...
    <>
      <main className='relative flex-1 p-6 pt-28'>
        ...
        <header className='flex flex-col gap-3 md:flex-row md:justify-between md:items-center'>
          <h2 className='text-sl font-medium md:text-xl md:font-semibold'>{user && user.role == "Admin" ? "Book Management" : "Books"}</h2>
          <div className='flex flex-col lg:flex-row space-y-4 lg:space-y-0 g:space-x-4'>
            ...
            <input type="text" placeholder='Search books...' className='w-full sm-52 border p-2 border-gray-300 rounded-md'
