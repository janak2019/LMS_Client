...
const Header = () => {
  ...
  useEffect(() => {
    const updateDateTime = () => {
      ...
      const ampm = now.getHours() >= 12 ? "PM" : "AM"; setCurrentTime(`${hours}: ${minutes} ${ampm}`);
  ...
  return (
    <header
      ...
      {/* Left side content */}
      < div > <UserIcon className='h-6 w-6' />
        <div className='flex flex-col'>
          <span className='text-sm font-medium sm-text-xl sm:font-semibold'>
            {user && user.name}</span>
